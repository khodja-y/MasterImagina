...
to search
  ...
  while [get-patch current != mygoal][
    ...
    if(get-patch current = mygoal)[
      print "success!!"
      draw-path current
      stop
    ]
    set in-nodes merge-nodes generate sort-nodesin-nodes
    if(empty? in-nodes)[
      print "echec!!"
      draw-path current
      stop
    ]
    let tmp first in-nodes
    let s remove-nodes tmp in-nodes
    if (not member? tmp out-nodes)[
      set current tmp
      set out-nodes fput current out-nodes
    ]
