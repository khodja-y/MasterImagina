...
to compute-repulsif1

  ask patches[
  if(goal = 0)[
      let old-pot repulsif
      let pot max-one-of neighbors4 [repulsif]
      let p ([repulsif] of pot) + 1
      if (repulsif >= p and p != old-pot)[
        set repulsif p
        set pcolor scale-color red repulsif 0 (potential-level * 1.2)
        ifelse (repulsif > 0)[
          set repulsif 0
        ][
          set continue true
...
to go-goal
  if goal != 1 [
      ...
      let path max-one-of way [potential + repulsif]
      if path != nobody [
