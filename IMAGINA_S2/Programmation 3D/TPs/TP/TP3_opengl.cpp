...
void surfaceCylindrique(Point* pointControlBezier, Point p0, Point p1, long nbControlPoint, double nbU, double nbV, Point* resultat){

  double x, y , z;
  Point pu, p;
  double u;
  Vector n;
  Point* nvPointControl = new Point[nbControlPoint];
  long nbNvControl = nbControlPoint;

  bezierCurveByBernstein(pointControlBezier, nbControlPoint, nbV, resultat);

  for(int i=0; i<nbV; i++){
    n = Vector(p1.getX()-p0.getX(),p1.getY()-p0.getY(),p1.getZ()-p0.getZ());
    x = resultat[i].getX()+ n.getX();
    y = resultat[i].getY()+ n.getY();
    z = resultat[i].getZ()+ n.getZ();

    p = Point(x,y,z);
    drawLine(resultat[i],p);
  }


  for(int j=0; j<nbControlPoint; j++){

      x = pointControlBezier[j].getX();
      y = pointControlBezier[j].getY();
      z = pointControlBezier[j].getZ();
  ...
  for(int i=0; i<nbU; i++){
    ...
    pu.setX(p0.getX()*(1-u) + p1.getX()*u);
    pu.setY(p0.getY()*(1-u) + p1.getY()*u);
    pu.setZ(p0.getZ()*(1-u) + p1.getZ()*u);

    n = Vector(pu.getX()-p0.getX(),pu.getY()-p0.getY(),pu.getZ()-p0.getZ());

    for(int j=0; j<nbNvControl; j++){

      x = n.getX() + nvPointControl[j].getX();
      y = n.getY() + nvPointControl[j].getY();
      z = n.getZ() + nvPointControl[j].getZ();
    ...
    bezierCurveByBernstein(nvPointControl, nbNvControl, nbV, resultat);
    drawCurve(resultat,nbV);
...
void render_scene()
{
  ...
  surfaceCylindrique(tabControlPoint, a, f, nbControlPoint, nbU, nbV, tab);
