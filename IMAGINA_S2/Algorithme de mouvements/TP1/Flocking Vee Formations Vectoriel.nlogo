...
to-report adjust ;; turtle
  ...
  if closest-distance > updraft-distance [
    set v1 vectFromAngle (towards closest-neighbor) (closest-distance)

    ;;turn-towards (towards closest-neighbor) ;; return an angle
  ...
  if any? visible-neighbors in-cone vision-distance obstruction-cone [
    set v2 vectFromAngle (random-float (max-turn * 2) - max-turn) (vision-distance)
  ...
  set v3 vectFromAngle ([heading] of closest-neighbor ) (distance closest-neighbor)
  ;; turn-towards [heading] of closest-neighbor
  set happy? true
  report v3


;  let va multiplyScalarvect factor-v1 v1
;  let vs multiplyScalarvect factor-v2 v2
;  let vc multiplyScalarvect factor-v3 v3
;
