...
turtles-own [mygoal in-nodes out-nodes current path chemin]
...
to search
  ...
  let indice 1
  while [get-patch current != mygoal and first path < 1] [
   search-step
  ]
  print "Success!!"

  ifelse(get-patch current = mygoal)[
    set indice 1
  ][
    set indice ([path] of get-patch current + 1)
  ]


  draw-path current indice

  while[patch-here != mygoal][
    let p min-one-of turtles with [path > 0][path]
    face p
    fd 1
  ]
...
to draw-path [node indice]
  let i indice
  while [node != []] [
    set path fput node pat
