...
to flock  ;; turtle procedure
  ifelse color = green
   [find-flockmates-green]
  [if color = blue
    [find-flockmates-blue]
  ]
  if any? flockmates
...
to find-flockmates-blue  ;; turtle procedure
  set flockmates other turtles in-radius vision with [color = blue]
end

to find-flockmates-green  ;; turtle procedure
  let oBird one-of other turtles in-radius vision with [color = orange]
  ifelse oBird = nobody
   [set flockmates other turtles in-radius vision with [color = green] ]
  [if any? flockmates
    [find-nearest-neighbor
     if distance nearest-neighbor < minimum-separation
          [turn-away ([heading] of nearest-neighbor) 70 ] ]
  ]
end
