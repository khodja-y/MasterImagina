...
Point* sommetCylindre(long nbrMeridien, long hauteur, long r){
  double angle = 0;
  double xp = 0, yp = 0, zp = 0;
  Point p;
  Point* resultat = new Point[nbrMeridien*2];
  long cpt = 0;
...
void affichageCylindre(Point* sommets, long nbrMeridien){

  Point* poly1 = new Point[nbrMeridien];
  Point* poly2 = new Point[nbrMeridien];

  long cpt = 0;

  for(int i=0; i<nbrMeridien*2; i+=2){
    poly1[cpt] = sommets[i];
    poly2[cpt] = sommets[i+1];
    cpt++;
  }
  ...
  for(int i=0; i<nbrMeridien; i++){
    glVertex3f(poly1[i].getX(), poly1[i].getY(), poly1[i].getZ());
  }
  glEnd();
  ...
  glColor3f(0.0F, 1.0F, 0.0F);
  for(int i=0; i<nbrMeridien; i++){
    glVertex3f(poly2[i].getX(), poly2[i].getY(), poly2[i].getZ());
  }
  glEnd();
...
void render_scene()
{
  ...
  long hauteur = 20, rayon = 10;
  Point* sommets = new Point[meridien*2];

  

  sommets = sommetCylindre(meridien, hauteur, rayon);

  for(int i=0; i<meridien*2; i+=2){
    printf("p: %f %f %f\n", sommets[i].getX(), sommets[i].getY(), sommets[i].getZ());
    printf("p1: %f %f %f\n", sommets[i+1].getX(), sommets[i+1].getY(), sommets[i+1].getZ());
  }

  glRotated(angle,1,0,0);
  glScaled(0.1,0.1,0.1);

  affichageCylindre(sommets, meridien);
