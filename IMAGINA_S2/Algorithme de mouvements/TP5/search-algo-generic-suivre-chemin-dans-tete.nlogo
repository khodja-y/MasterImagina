...
turtles-own [mygoal in-nodes out-nodes current path ]
...
to search
  ...
  let indice 1
  while [get-patch current != mygoal and [path] of get-patch current < 1] [
   search-step
  ]
  print "Success!!"
  ...
  while[patch-here != mygoal][
    let p min-one-of neighbors with [path > 0][path]
    face p
    fd 1
  ]
