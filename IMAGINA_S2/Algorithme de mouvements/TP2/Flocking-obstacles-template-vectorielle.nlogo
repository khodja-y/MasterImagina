...
to avoid-obstacles
  ;; à créer
  set obstacles patches in-cone vision angle-avoidance with [pcolor != black]
  if(any? obstacles)
  [
    let a angleFromVect vectDirect
    turn-towards a max-turn-angle
    ;;fuite
  ]
...
to-report vectFlock
  ...
  let vc multiplyScalarvect factor-coherent vectCohere
...
to-report vectObstacle
  let vo 0
  find-nearest-obstacle
  ifelse nearest-obstacle = nobody
  [set vo VectFromAngle heading 0]
  [set vo VectFromAngle (towards nearest-obstacle - 180 ) (1 / distance nearest-obstacle)]
  report vo
...
to-report vectDirect
  find-nearest-obstacle
  set coef-obstacle (1 / (distance nearest-obstacle * distance nearest-obstacle))

  let coef-flock (1 - coef-obstacle)
  let vf vectFromAngle heading 0
  find-flockmates
  if (any? flockmates)[
    set vf multiplyScalarvect coef-flock vectFlock
  ]

  let vo multiplyScalarvect coef-obstacle vectObstacle


  let vr additionvect vf vo
  report vr
end
...
to fuite
  let obstacles-in-front obstacles in-cone 3 angle-fuite
