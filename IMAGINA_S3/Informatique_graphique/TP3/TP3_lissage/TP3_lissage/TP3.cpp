...
void calc_uniform_mean_curvature (const std::vector<glm::vec3> & vertices,
    ...
    for(int i=0; i < vertices.size(); i++){
        std::vector<unsigned short> ring = one_ring[i];
        glm::vec3 somme(0.,0.,0.);

        for(int j=0; j < ring.size(); j++){

            somme += vertices[ring[j]];
            
        }

        somme /= ring.size();

        //cout << somme.x << "-" << somme.y << "-" << somme.z << endl;
        somme -= vertices[i];

        float taille = sqrt(pow(somme.x, 2) + pow(somme.y, 2) + pow(somme.z, 2));

        Lu.push_back(somme);

        //cout << "Lu du (" << vertices[i].x << ", " << vertices[i].y << ", " << vertices[i].z << ") = " << taille/2 << endl;        
    }
...
void uniform_smooth(unsigned int _iters,
    ...
    for(int j=0; j < _iters; j++){
        ...
        for(int i=0; i < vertices.size(); i++){
            ...
            cout << j << " - (" << vertices[i].x << ", " << vertices[i].y << ", " << vertices[i].z << ") - " << endl;
        }
    }

}

float plus_petit(vector<float> normes){
...
void calc_triangle_quality(const std::vector< std::vector<unsigned short> > & triangles,
                           const std::vector<glm::vec3> & vertices,
                           std::vector<unsigned short> & tshape){
          

    for(int i=0; i < triangles.size(); i++){

        glm::vec3 a = vertices[triangles[i][0]];
        glm::vec3 b = vertices[triangles[i][1]];
        glm::vec3 c = vertices[triangles[i][2]];
        
        glm::vec3 ab(b.x - a.x, b.y - a.y, b.z - a.z);
        glm::vec3 bc(c.x - b.x, c.y - b.y, c.z - b.z);
        glm::vec3 ca(a.x - c.x, a.y - c.y, a.z - c.z);

        glm::vec3 abbc(ab.y*bc.z - ab.z*bc.y, ab.z*bc.x - ab.x*bc.z, ab.x*bc.y - ab.y*bc.x);

        float normeabbc = sqrt(pow(abbc.x,2) + pow(abbc.y,2) + pow(abbc.z,2));

        float nab = sqrt(pow(ab.x,2) + pow(ab.y,2) + pow(ab.z,2));
        float nbc = sqrt(pow(bc.x,2) + pow(bc.y,2) + pow(bc.z,2));
        float nca = sqrt(pow(ca.x,2) + pow(ca.y,2) + pow(ca.z,2));

        float r = (nab * nbc * nca) / (2 * normeabbc);

        vector<float> normes;
        normes.push_back(nab);
        normes.push_back(nbc);
        normes.push_back(nca);

        tshape.push_back(r/plus_petit(normes));


    }
}

void calc_weight(const std::vector< std::vector<unsigned short> > & triangles,
                 const std::vector<glm::vec3> & vertices,
...
int main( void )
{
    ...
    uniform_smooth(4 , indexed_vertices, triangles);
    ...
    std::vector<glm::vec3> Lu;
    calc_uniform_mean_curvature(indexed_vertices, triangles, Lu);
